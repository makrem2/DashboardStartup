...
export class AuthAdminService {
  ...
  checkEmail(data: any) {
    return this.http.post(API_URL+'auth/checkEmail', data);
  }

  resetPassword(data: any) {
    return this.http.post(API_URL+'auth/resetPassword', data);
  }
  ...
  public addSession(type: string, username: string) {
    ...
    this.http
      .post(
        API_URL+'Session/session/add-session',
        request_body
      )
      .subscribe((data) => {
        if (data != null) {
  ...
  public deleteSession(id: number) {
    let url: string =
    API_URL+'Session/session/delete-session/' + id;
